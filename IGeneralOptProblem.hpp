...
protected:
  ...
  struct TOptFunction
  {
    ...
    int mDimension;
    // Minimizer
    double mMinimumPoint;
    // Minimum value
    double mMinimumValue;
    // Maximizer
    double mMaximumPoint;
    // Maximum value
  ...
  */
  ...
  void SetFunctionMin(uint index, double minPoint, double minValue);
  /// Get the global minimizer for function number index
  /// If not specified, throws an exception
  double GetMinPoint(uint index) const;
  ...
  void SetFunctionMax(uint index, double maxPoint, double maxValue);
  /// Get the global maximizer for function number index
  /// If not specified, throws an exception
  double GetMaxPoint(uint index) const;
  ...
  IGeneralOptProblem();
public:
  IGeneralOptProblem(int dim, double loBound, double upBound, int probIndex = -1);
  /// Get dimension
